...
**實作備註**
- TBD（使用 `/process-task-list` 實作時才填寫）
...
- **實作要點**：具體的實作步驟（條列式）
- **相關檔案**：該任務需要修改的檔案清單（每個任務獨立，不使用全域區塊，**使用列表格式，禁止使用表格**）
- **完成檢查**：1-5 條簡要的檢查要點，說明如何確認該任務已完成（使用純條列 `-`，不使用 checkbox `- [ ]`）
- **實作備註**：**生成時填入 `- TBD`**，使用 `/process-task-list` 實作時才填寫實際內容

**注意**：「執行驗收測試」和「更新專案文件」兩個固定任務不包含「完成檢查」章節。

### 任務標題原則

- **禁止使用編號前綴**：不要在任務標題前加上「任務 1：」、「任務 2：」等前綴
- 使用描述性標題，讓標題本身說明任務內容
- 概要和細節的標題必須完全一致
...
**錯誤範例：**
- `- [ ] 任務 1：資料庫結構調整與型別定義` ❌
- `- [ ] Task 2: 實作 OAuth 2.0 認證流程` ❌

### 實作備註填寫指引

**重要：生成 implementation.md 時，實作備註必須填入 `- TBD（使用 /process-task-list 實作時才填寫）`。以下指引是給使用 `/process-task-list` 執行任務的開發者參考的。**
